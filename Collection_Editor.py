...
class BHiMAX_SelectObjectOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if 0 <= self.object_index < len(collection.objects):
            ...
            bpy.context.view_layer.objects.active = obj_item.object
            obj_item.object.select_set(True)
    ...
    def draw(self, context):
        ...
        for i, collection in enumerate(context.scene.bhimax_collections):
            ...
            if collection.is_selected:
                ...
                # Add object to collection button (only text)
                add_object_button = row.operator("bhimax.add_object", text="Add Object")
                add_object_button.collection_index = i
                
                # Remove collection button (icon)
                remove_button = row.operator("bhimax.remove_collection", text="", icon='X')
                ...
                for j, obj_item in enumerate(collection.objects):
                    ...
                    select_object_button = subrow.operator("bhimax.select_object", text="Select")
